...
int main()
{
    ...
    for (i=0;i<n-1;i++)
    {
        for (j=0;j<m-1;j++)
        {
            ...
            {
                ulx=i,uly=j,urx=i+1,ury=j+1,dlx=i+1,dly=j,drx=i+1,dry=j+1;
    ...
    while(1)
    {
        ...
        {
            //right
            if (box[urx][ury+1]=='x' && box[drx][dry+1]=='x' && ury<m-1 && dry<m-1)
        ...
        if (dlx==n-1 && drx==n-1)
            {ans = 1; break;}
        else if (dlx<n-1 && drx<n-1 && ury<m-1 && dry<m-1 && uly>0 && dly>0 && urx>0 && ulx>0 && box[dlx+1][dly]!='x' && box[drx+1][dry]!='x' && box[urx][ury+1]!='x' && box[drx][dry+1]!='x' && box[ulx][uly-1]!='x' && box[dlx][dly-1]!='x' && box[ulx-1][uly]!='x' && box[urx-1][dly]!='x')
        {
            stop=0;
            for (j=0;j<m;j++)
            {
